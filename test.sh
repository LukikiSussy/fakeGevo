...
for j in "${!files[@]}"
do
        alllinks=($( find . -type f -wholename "${files[j]}" -print0 |  xargs -0 -r grep -ohG "responsive/360[^\" ]*"))
        alllinkss=($( find . -type f -wholename "${files[j]}" -print0 |  xargs -0 -r grep -ohG "../en[^\" ]*"))
        goodurl=($( find . -type f -wholename "${files[j]}" -print0 | xargs -0 -r grep -ohG "responsive/360[^\" ]*" | sed -e 's|responsive/360|responsive/720|'))
